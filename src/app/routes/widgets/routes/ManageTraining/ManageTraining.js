...
function App({ currentBaseUrl }) {
    ...
    apigetUrl(`/auth/login/roles-members/${roleId}`)
      .then((res) => {
        ...
        if (res.data.responseCode === "200") {
          let userDetails = res.data.dataList
          let Defaultuser={
            keyCloakId: "0",
            username: "Select All Users"
          }
          arrayData.push(Defaultuser)
          for(let i=0;i<userDetails.length;i++){
            let userJson={keyCloakId: userDetails[i].keyCloakId,username: userDetails[i].username}
            arrayData.push(userJson)
          }
          setState((prevState) => ({
            ...prevState,
            userData: arrayData,
  ...
  const handleDelete = (id) => {
    ...
    let removeUserData = state.assignUserData.filter(
      (item) => !id.includes(item.userId)
    );
    setState((prevState) => ({
      ...prevState,
      userSelectedData: removeSlectedData,
      userData: addRemoveUserData,
      assignUserData: removeUserData
    }));
  };


  const handleUserChange = (e, value,reason)=>{
    let arrayData = []
    for(let i=0;i<value.length;i++)
    {
    if(value[i].username ==="Select All Users"){selectAllUserFunction()}
    else{
      if(reason==="remove-option"){arrayData.push(value[i])}
      else if(reason==="select-option"||reason==="create-option"){
        if (state.userFilter.length == 0) { arrayData.push(value[i]) }
        else {
        arrayData = state.userFilter
        var unique = "Yes";
        for (var z = 0; z < arrayData.length; z++) {
          if (arrayData[z].keyCloakId === value[i].keyCloakId) { unique = "No"; }
        }
        if (unique == "Yes") { arrayData.push(value[i]) }
      }
    }
  }
    }
    let status=false
    if(arrayData.length===0){status=false}
    else{status=true}
    setState((prevState) => ({
      ...prevState,
      userFilter: arrayData,
      isUserChangeActive:status
    }));
  }

  const selectAllUserFunction = ()=>{
    let selectedData = state.userList
    let userSelectedArray = state.userSelectedData
    let arrayData=[]
    let dataStructure=[]

    if (state.userSelectedData.length == 0) { 
      for (var j = 0; j < selectedData.length; j++) { 
        arrayData.push(selectedData[j])
      let data = {
        userId: selectedData[j].keyCloakId,
        courseId: state.currentEditId
      }
      dataStructure.push(data)
      }
    }
    else {
      arrayData = userSelectedArray
      for (var j = 0; j < selectedData.length; j++) {
        var unique = "Yes";
        for (var z = 0; z < arrayData.length; z++) {
          if (arrayData[z].keyCloakId === selectedData[j].keyCloakId) { unique = "No";  }
        }
        if (unique == "Yes") { 
          arrayData.push(selectedData[j])
          let data = {
    ...
    setState((prevState) => ({
      ...prevState,
      userSelectedData: arrayData,
      assignUserData:dataStructure,
...
const handleAdd = ()=>{
  let selectedData = state.userFilter
    let userSelectedArray = state.userSelectedData
    let arrayData=[]
    let dataStructure=[]
...
const handleEditFormSubmit = async()=>{
  const result = await apipostUrl(`/otm/trainings/assign`, state.assignUserData);
  if (result.status === 200) {
    ippNotify.success("Training Successfully Assigned!");
    setState((prevState) => ({
      ...prevState,
      editForm_DialogOpen: false,
      userSelectedData:[],
  ...
  const editTableContent = (
    <>
      <DialogTitle>
      <IntlMessages id="ManageTraining.Edit.Modal.Title"/>
      </DialogTitle>
      <DialogContent>
        <div className="row">
        ...
        <InputField
          ...
          id="name"
          className="mb-3"
          label={<IntlMessages id="ManageTraining.Edit.Modal.ShortName"/>}
          name="name"
          value={state.shortTraining}
          fullWidth
        ...
        <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      variant="outlined"
                      label={<IntlMessages id="ManageTraining.Edit.Modal.Role"/>}
      ...
      <Autocomplete
                  multiple
                  limitTags={0}
                  onChange={(e, value,reason) => handleUserChange(e, value,reason)}
     ...
     >
      ...
      <DialogActions>
        ...
        {(state.userSelectedData.length >= 1 ) ? (
              <>
        <InputSubmitButton
  ...
  const callUserApi = async () => {
    const result= await apigetUrl(`/auth/users?page=1&limit=1000`)
    if(result.data.responseCode === "200"){
     let user = result.data.dataList;
     setState((prevState) => ({
       ...prevState,
       userData: user,
     }));
  ...
  const onTableViewButtonClick = (event) => {
    ...
    let trainingData = state.data
    let trainingFilter = trainingData.filter(a=>a.id===(parseInt(currentId)))
    let shortName=""
    if(trainingFilter.length===0){}
    else{
      shortName=trainingFilter[0].shortName
    }
    setState((prevState) => ({
      ...prevState,
      shortTraining:shortName,
      editForm_DialogOpen: true,
