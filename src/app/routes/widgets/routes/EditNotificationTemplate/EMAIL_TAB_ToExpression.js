...
function EMAIL_TAB_ToExpression(props) {
  const {
    key,
    id,
    name,
  ...
  useEffect(() => {
    localforage
      .setItem("EMAIL_TAB_ToExpression", {
        toExpression: state.validation.toExpression,
        name: "toExpression",
      })
      .then(function() {
        return localforage.getItem("EMAIL_TAB_ToExpression");
      })
      .then(function(value) {
        // we got our value
        //console.log("EMAIL_TAB_ToExpression Data stored", value);
      })
      .catch(function(err) {
        // we got an error
...
EMAIL_TAB_ToExpression.propTypes = {};

export default EMAIL_TAB_ToExpression;
