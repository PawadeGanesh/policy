...
export default function FixedNotifyLanguageTextInput(props) {
  const {
    id,
    name,
    label,
  ...
  useEffect(() => {
    localforage
      .setItem("SMS_TAB_FixedNotifyLanguage", {
        notifyLanguage: state.validation.notifyLanguage,
        name: "notifyLanguage",
      })
      .then(function() {
        return localforage.getItem("SMS_TAB_FixedNotifyLanguage");
      })
      .then(function(value) {
        // we got our value
        //console.log("SMS_TAB_FixedNotifyLanguage Data stored", value);
      })
      .catch(function(err) {
        // we got an error
