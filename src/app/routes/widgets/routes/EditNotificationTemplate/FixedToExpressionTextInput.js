...
export default function FixedToExpressionTextInput(props) {
  const {
    id,
    name,
    label,
  ...
  useEffect(() => {
    localforage
      .setItem("SMS_TAB_FixedToExpression", {
        toExpression: state.validation.toExpression,
        name: "toExpression",
      })
      .then(function() {
        return localforage.getItem("SMS_TAB_FixedToExpression");
      })
      .then(function(value) {
        // we got our value
        //console.log("SMS_TAB_FixedToExpression Data stored", value);
      })
      .catch(function(err) {
        // we got an error
