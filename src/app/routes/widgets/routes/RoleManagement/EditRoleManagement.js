...
const EditRoleManagement = ({
  ...
  const getRoleDetails = async (menu) => {
    const result = await apigetUrl(`/auth/login/roles/${selectedId}`);
    if (result.data.responseCode === "200") {
      let resultData = result.data;
      let menuId = resultData.roles;
      for (var i = 0; i < menuId.length; i++) {
        let id = menuId[i];
        editMenuStructure(id, menu);
      }
      setState((prevState) => ({
        ...prevState,
        roleName: resultData.name,
        description: resultData.description,
      }));
    }
  };

  //Check menu which are selected
  const editMenuStructure = (id, menu) => {
    const checkedBoxesArray = [];
    let dataFilter = menu.filter((a) => a.name === id);
    if (dataFilter.length > 0) {
      checkedBoxesArray.push({
        id: dataFilter[0].id,
        name: dataFilter[0].name,
        parent: dataFilter[0].parent,
      });
      editCheckBoxState(checkedBoxesArray);
    } else {
      menu.map((item) => {
        let dataItem = undefined;
        if (item.children && item.children.length > 0) {
          dataItem = editMenuStructure(id, item.children);
        }
      });
    }
  };

  //load the menu selected in checkbox
  const editCheckBoxState = (data) => {
  ...
  //Display list of menu present
  const getMenuDetails = async () => {
    const result = await apigetUrl(`/auth/menus`);
    if (result.data.responseCode === "200") {
      let menu = result.data.dataList;
      setState((prevState) => ({
        ...prevState,
        ...
        <div className="cardBox">
          <div className="row">
            <div className="col-lg-6">
              <InputField
                ...
                autoFocus
                id="roleName"
                label={<IntlMessages id="RoleManagement.master.edit.role" />}
                name="roleName"
            ...
            <div className="col-lg-6">
              <InputField
                ...
                id="description"
                label={
                  <IntlMessages id="RoleManagement.master.edit.description" />
                }
                name="description"
          ...
          <div className="row">
            <div className="col-lg-12">
              <h3>
                <IntlMessages id="RoleManagement.master.edit.menu" />
              </h3>
            </div>
          </div>
          <div className="row">
          ...
          <div className="text-center">
            ...
            <InputCancelButton onClick={(e) => closeEditRoleManagement(e)} />
