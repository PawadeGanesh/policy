...
const perPageURL = `/notify/notifications?mode=push`;
                ...
                <InputDatePicker
                  ...
                  customInput={
                    <TextField
                      style={{ width: "100%" }}
                      label={
                        <IntlMessages id="notificationdetails.report.tableheader.push.advancedsearch.StartDate" />
                      }
                      variant="outlined"
                    />
                  }
                ...
                <InputDatePicker
                  ...
                  customInput={
                    <TextField
                      style={{ width: "100%" }}
                      label={
                        <IntlMessages id="notificationdetails.report.tableheader.push.advancedsearch.EndDate" />
                      }
                      variant="outlined"
                    />
                  }
...
const PushTable = ({ getLoadingValue }) => {
    ...
    headCells: [
      {
        id: "toNumber",
        isActive: true,
        label: "notificationdetails.report.tableheader.push.ToNumber.label",
      },
      {
        id: "content",
        isActive: false,
        label: "notificationdetails.report.tableheader.push.Content.label",
      },
      {
        id: "status",
        isActive: true,
        label: "notificationdetails.report.tableheader.push.status.label",
      },
      {
        id: "created",
        isActive: true,
        label: "notificationdetails.report.tableheader.push.createdDate.label",
      },
      {
        id: "sent",
        isActive: true,
        label: "notificationdetails.report.tableheader.push.sentDate.label",
      },
    ],
  });
  ...
  const callLocalBaseURL = async () => {
    const result = await apigetUrl(
      `/notify/notifications?mode=push&page=${state.page}&limit=${state.limit}&sortBy=${state.sortBy}&sortType=${state.sortType}`
    );
  ...
  const requestPageLimitCountChange = async (count) => {
    ...
    setCurrentUrl(pageURL);
    const result = await apigetUrl(
      `/notify/notifications?mode=push&page=${1}&limit=${count}&sortBy=${
        state.sortBy
  ...
  const getPageFromBackEnd = async (
    ...
    pageURL.search = apiCallParams.toString();
    const result = await apigetUrl(
      `/notify/notifications?mode=push&page=${pageNumber}&limit=${limit}&sortBy=${state.sortBy}&sortType=${state.sortType}`
    );
    if (result.data.responseCode === "200") {
      setTimeout(() => {
  ...
  const callResetData = async () => {
    ...
    pageURL.search = apiCallParams.toString();
    const result = await apigetUrl(
      `/notify/notifications?mode=push&page=${1}&limit=${state.limit}&sortBy=${
        state.sortBy
      }&sortType=${state.sortType}`
    );
    if (result.data.responseCode === "200") {
