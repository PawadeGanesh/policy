...
const perPageURL = `/notify/notifications?mode=inapp`;
                ...
                <InputDatePicker
                  ...
                  customInput={
                    <TextField
                      style={{ width: "100%" }}
                      label={
                        <IntlMessages id="notificationdetails.report.tableheader.inapp.advancedsearch.StartDate" />
                      }
                      variant="outlined"
                    />
                  }
                ...
                <InputDatePicker
                  ...
                  customInput={
                    <TextField
                      style={{ width: "100%" }}
                      label={
                        <IntlMessages id="notificationdetails.report.tableheader.inapp.advancedsearch.EndDate" />
                      }
                      variant="outlined"
                    />
                  }
...
const InAppTable = ({ getLoadingValue }) => {
    ...
    headCells: [
      {
        id: "title",
        isActive: true,
        label: "notificationdetails.report.tableheader.inapp.Title.label",
      },
      {
        id: "content",
        isActive: false,
        label: "notificationdetails.report.tableheader.inapp.Content.label",
      },
      {
        id: "status",
        isActive: true,
  ...
  const callLocalBaseURL = async () => {
    const result = await apigetUrl(
      `/notify/notifications?mode=inapp&page=${state.page}&limit=${state.limit}&sortBy=${state.sortBy}&sortType=${state.sortType}`
    );
  ...
  const requestPageLimitCountChange = async (count) => {
    ...
    setCurrentUrl(pageURL);
    const result = await apigetUrl(
      `/notify/notifications?mode=inapp&page=${1}&limit=${count}&sortBy=${
        state.sortBy
  ...
  const getPageFromBackEnd = async (
    ...
    pageURL.search = apiCallParams.toString();
    const result = await apigetUrl(
      `/notify/notifications?mode=inapp&page=${pageNumber}&limit=${limit}&sortBy=${state.sortBy}&sortType=${state.sortType}`
    );
    if (result.data.responseCode === "200") {
      setTimeout(() => {
