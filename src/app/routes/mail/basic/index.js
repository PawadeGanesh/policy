...
const Mail = () => {
  ...
  const [noContentFoundMessage, setNoContentFoundMessage] = useState(
    "No mail found in selected folder"
  );
  ...
  const [folderMails, setFolderMails] = useState(
    mails.filter((mail) => mail.folder === 0)
  );
  ...
  const getNavFolders = () => {
    return folders.map((folder, index) => (
      <li
        key={index}
        onClick={() => {
          const filterMails = allMail.filter(
      ...
      >
        <span
          className={`jr-link ${selectedFolder === folder.id ? "active" : ""}`}
        >
  ...
  const onSearchMail = (searchText) => {
    if (searchText === "") {
      ...
      const searchMails = allMail.filter(
        (mail) =>
          !mail.deleted &&
  ...
  const getMailActions = () => {
    return (
      <div>
        ...
        <IconButton onClick={onLabelSelect} className="icon-btn">
  ...
  const onStartSelect = (data) => {
    ...
    setShowMessage(true);
    setFolderMails(
      folderMails.map((mail) => (mail.id === data.id ? data : mail))
    );
  };
  ...
  const onImportantSelect = (data) => {
    ...
    setAlertMessage(
      data.important ? "Mail Mark as Important" : "Mail Remove as Important"
    );
    setShowMessage(true);
    setFolderMails(
      folderMails.map((mail) => (mail.id === data.id ? data : mail))
                ...
                {currentMail === null ? (
                  <div className="d-flex">
                    <Checkbox
                      color="primary"
                      indeterminate={
                        selectedMails > 0 && selectedMails < folderMails.length
                      }
